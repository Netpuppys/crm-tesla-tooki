...
const RequestDeletePopUp = ({ deleteId, setDeleteId, setNewDriverList }) => {
  ...
  const handleClick = () => {
    if (typeof deleteId === "number") {
      axiosInstance
      .post(`/all/status/driver/deleterequest/${deleteId}/`, {})
      .then(res => {
        console.log(res)
        setTimer(5)
        setNewDriverList(prev => prev.filter(item => item.driver !== deleteId))
        setTimeout(() => {
          setDeleteId(null)
        }, 5000)
      })
